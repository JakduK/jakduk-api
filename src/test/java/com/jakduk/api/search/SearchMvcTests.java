				...
				new ArticleSource() {
					{
						...
						setHighlight(
							new HashMap<String, List<String>>() {{
								put("subject",
									Arrays.asList("Summernote insert-image <div class=\\\"description\\\">테스트</div>"));
			...
			Arrays.asList(
				new EsGallerySource() {{
					...
					setHighlight(
						new HashMap<String, List<String>>() {{
							put("name",
								Arrays.asList("shortContent와 썸네일URL 추가 <div class=\\\"description\\\">테스트</div>"));
						}}
					);
				}}
			)
			...
			.andDo(
				document("search-unified",
					...
					responseFields(
						...
						fieldWithPath("articleResult.articles.[].category").type(JsonFieldType.STRING)
							.description("말머리"),
						subsectionWithPath("articleResult.articles.[].writer").type(JsonFieldType.OBJECT)
							.description("글쓴이"),
						subsectionWithPath("articleResult.articles.[].galleries").type(JsonFieldType.ARRAY)
							.description("그림 목록"),
						fieldWithPath("articleResult.articles.[].score").type(JsonFieldType.NUMBER)
							.description("매칭 점수"),
						subsectionWithPath("articleResult.articles.[].highlight").type(JsonFieldType.OBJECT)
							.description("매칭 단어 하이라이트"),
						...
						subsectionWithPath("commentResult.comments.[].article").type(JsonFieldType.OBJECT)
							.description("연관 글 객체"),
						subsectionWithPath("commentResult.comments.[].writer").type(JsonFieldType.OBJECT)
							.description("글쓴이"),
						fieldWithPath("commentResult.comments.[].score").type(JsonFieldType.NUMBER)
							.description("매칭 점수"),
						subsectionWithPath("commentResult.comments.[].highlight").type(JsonFieldType.OBJECT)
							.description("매칭 단어 하이라이트"),
						...
						subsectionWithPath("galleryResult.galleries").type(JsonFieldType.ARRAY)
							.description("매칭되는 그림 목록"),
						fieldWithPath("galleryResult.galleries.[].id").type(JsonFieldType.STRING).description("그림 ID"),
						fieldWithPath("galleryResult.galleries.[].writer").type(JsonFieldType.OBJECT)
							.description("글쓴이"),
						fieldWithPath("galleryResult.galleries.[].score").type(JsonFieldType.NUMBER)
							.description("매칭 점수"),
						subsectionWithPath("galleryResult.galleries.[].highlight").type(JsonFieldType.OBJECT)
							.description("매칭 단어 하이라이트")
					)
				));
	}
