...
public class ArticleRepositoryTests {
	...
	public void setUp() {
		...
		randomArticle = repository.findTopByIdLessThanEqualOrderByIdDesc(
	...
	public void findUsersFeelingCountTest() {
		List<BoardFeelingCount> feelingCounts = repository.findUsersFeelingCount(
			Arrays.asList(new ObjectId(randomArticle.getId())));
		...
		assertTrue(boardFeelingCount.getUsersLikingCount()
			.equals(
				CollectionUtils.isEmpty(randomArticle.getUsersLiking()) ? 0 : randomArticle.getUsersLiking().size()));
		assertTrue(boardFeelingCount.getUsersDislikingCount()
			.equals(CollectionUtils.isEmpty(randomArticle.getUsersDisliking()) ? 0 :
				randomArticle.getUsersDisliking().size()));
	}
	...
	public void findTopLikes() {
		...
		List<BoardTop> topLikes = repository.findTopLikes(Constants.BOARD_TYPE.FOOTBALL,
