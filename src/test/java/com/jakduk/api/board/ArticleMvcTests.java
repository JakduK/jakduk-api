...
public class ArticleMvcTests {
	...
	public static void setUp() {
		...
		categoriesMap = categories.stream()
			.collect(
				Collectors.toMap(BoardCategory::getCode, boardCategory -> boardCategory.getNames().get(0).getName()));
		...
		article.setViews(15);
		article.setUsersLiking(
			Arrays.asList(new CommonFeelingUser("58ee4993807d713fa7735f1d", "566d68d5e4b0dfaaa5b98685", "test05")));
		article.setUsersDisliking(
			Arrays.asList(new CommonFeelingUser("58ee4993807d713fa7735f1d", "566d68d5e4b0dfaaa5b98685", "test05")));
			...
			.andDo(document("getArticles",
				...
				requestParameters(
					...
					parameterWithName("categoryCode").description(
						"(optional, default ALL) 말머리. board가 FREE 일때에는 무시된다. FOOTBALL, DEVELOPER 일 때에는 필수다.").optional()
				),
				responseFields(
					subsectionWithPath("categories").type(JsonFieldType.OBJECT)
						.description("말머리 맵. key는 말머리코드, value는 표시되는 이름(Locale 지원)"),
					...
					subsectionWithPath("notices").type(JsonFieldType.ARRAY)
						.description("공지글 목록. json 형식은 articles와 같음."),
	...
	public void getArticleDetailTest() throws Exception {
		...
		when(articleService.getArticleDetail(any(CommonWriter.class), any(Constants.BOARD_TYPE.class), anyInt(),
			anyBoolean()))
		...
		mvc.perform(
			...
			.andDo(document("getArticleDetail",
				...
				responseFields(
					...
					fieldWithPath("article.myFeeling").type(JsonFieldType.STRING)
					...
					subsectionWithPath("nextArticle").type(JsonFieldType.OBJECT)
						.description("다음 글. json 형식은 prevArticle과 같음."),
					...
					subsectionWithPath("latestArticlesByWriter.[].writer").type(JsonFieldType.OBJECT)
						.description("글쓴이"),
					fieldWithPath("latestArticlesByWriter.[].subject").type(JsonFieldType.STRING).description("글제목"),
					subsectionWithPath("latestArticlesByWriter.[].galleries").type(JsonFieldType.ARRAY)
						.description("그림 목록")
				)
			));
	}
	...
	public void writeArticleTest() throws Exception {
		...
		when(articleService.insertArticle(any(CommonWriter.class), any(Constants.BOARD_TYPE.class), anyString(),
			anyString(), anyString(),
			anyBoolean()))
			.thenReturn(article);
			...
			.andDo(
				document("getBoardCategories",
					...
					responseFields(
						...
						fieldWithPath("categories.[].names").type(JsonFieldType.ARRAY)
							.description("말머리 이름 목록(Locale 지원)"),
	...
	public void editArticleTest() throws Exception {
		...
		when(articleService.updateArticle(any(CommonWriter.class), any(Constants.BOARD_TYPE.class), anyInt(),
				...
				document("deleteArticle",
					...
					responseFields(
						fieldWithPath("result").type(JsonFieldType.STRING)
							.description("결과 타입. [ALL : 모두 지움, CONTENT : 글 본문만 지움(댓글 유지)]")
					)
				));
	}
	...
	public void getArticleFeelingUsersTest() throws Exception {
		...
		GetArticleFeelingUsersResponse expectResponse = new GetArticleFeelingUsersResponse(article.getSeq(),
			article.getUsersLiking(), article.getUsersDisliking());
	...
	private FieldDescriptor[] getWriteArticleFormDescriptor() {
		...
		return new FieldDescriptor[] {
			fieldWithPath("subject").type(JsonFieldType.STRING).description(
				"글 제목. " + userConstraints.descriptionsForProperty("subject")),
			fieldWithPath("content").type(JsonFieldType.STRING).description(
				"글 내용. " + userConstraints.descriptionsForProperty("content")),
