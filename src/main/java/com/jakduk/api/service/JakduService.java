...
public class JakduService {
	...
	public JakduSchedule findScheduleById(String id) {
		return jakduScheduleRepository.findById(id)
	...
	public Jakdu setMyJakdu(CommonWriter writer, MyJakduRequest myJakdu) {
		JakduSchedule jakduSchedule = jakduScheduleRepository.findById(myJakdu.getJakduScheduleId())
			.orElseThrow(() -> new ServiceException(ServiceError.NOT_FOUND_JAKDUSCHEDULE));

		if (Objects.isNull(jakduSchedule))
			throw new NoSuchElementException(
				JakdukUtils.getMessageSource("jakdu.msg.not.found.jakdu.schedule.exception"));

		JakduOnSchedule existJakdu = jakduRepository.findByUserIdAndWriter(writer.getUserId(),
			new ObjectId(jakduSchedule.getId()));

		if (Objects.nonNull(existJakdu))
			throw new ServiceException(ServiceError.INTERNAL_SERVER_ERROR,
				JakdukUtils.getMessageSource("jakdu.msg.already.join.jakdu.exception"));
	...
	public JakduOnSchedule getMyJakdu(String userId, String jakdukScheduleId) {
		JakduSchedule jakduSchedule = jakduScheduleRepository.findById(jakdukScheduleId)
			.orElseThrow(() -> new ServiceException(ServiceError.NOT_FOUND_JAKDUSCHEDULE));
	...
	public JakduComment setComment(CommonWriter writer, JakduCommentWriteRequest request) {
		JakduSchedule jakduSchedule = jakduScheduleRepository.findById(request.getId())
			.orElseThrow(() -> new ServiceException(ServiceError.NOT_FOUND_JAKDUSCHEDULE));
	...
	public JakduComment setJakduCommentFeeling(CommonWriter writer, String commentId, Constants.FEELING_TYPE feeling) {
		...
		JakduComment jakduComment = jakduCommentRepository.findById(commentId)
		...
		if (Objects.isNull(usersLiking))
			usersLiking = new ArrayList<>();
		if (Objects.isNull(usersDisliking))
