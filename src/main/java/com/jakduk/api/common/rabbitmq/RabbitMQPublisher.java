...
public class RabbitMQPublisher {
	...
	public void sendResetPassword(Locale locale, String recipientEmail, String host) {
		...
		String routingKey = rabbitmqProperties.getRoutingKeys()
			.get(EmailRoutingKey.EMAIL_RESET_PASSWORD.getRoutingKey());
		this.publishEmail(routingKey, emailPayload);
	}

	public void indexDocumentArticle(String id, Integer seq, String board, String category, CommonWriter writer,
		String subject,
	...
	public void indexDocumentComment(String id, ArticleItem articleItem, CommonWriter writer, String content,
	...
	public void deleteDocumentComment(String id) {
		String routingKey = rabbitmqProperties.getRoutingKeys()
			.get(ElasticsearchRoutingKey.ELASTICSEARCH_DELETE_DOCUMENT_ARTICLE_COMMENT.getRoutingKey());
		this.publishElasticsearch(routingKey, id);
	}
	...
	public void indexDocumentGallery(String id, CommonWriter writer, String name) {
		...
		String routingKey = rabbitmqProperties.getRoutingKeys()
			.get(ElasticsearchRoutingKey.ELASTICSEARCH_INDEX_DOCUMENT_GALLERY.getRoutingKey());
		this.publishElasticsearch(routingKey, esGallery);
	}

	public void deleteDocumentGallery(String id) {
		String routingKey = rabbitmqProperties.getRoutingKeys()
			.get(ElasticsearchRoutingKey.ELASTICSEARCH_DELETE_DOCUMENT_GALLERY.getRoutingKey());
		this.publishElasticsearch(routingKey, id);
	}
	...
	public void indexDocumentSearchWord(String word, CommonWriter writer) {
		...
		esSearchWord.setWriter(
			Objects.nonNull(writer) ? new SimpleWriter(writer.getUserId(), writer.getUsername()) : null);
		esSearchWord.setRegisterDate(LocalDateTime.now());

		String routingKey = rabbitmqProperties.getRoutingKeys()
			.get(ElasticsearchRoutingKey.ELASTICSEARCH_INDEX_DOCUMENT_SEARCH_WORD.getRoutingKey());
