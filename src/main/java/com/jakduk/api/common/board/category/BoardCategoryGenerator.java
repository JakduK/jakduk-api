...
public class BoardCategoryGenerator {
    ...
    public enum Category {
		...
		private static List<Category> FREE = Lists.newArrayList();
    ...
    public static List<BoardCategory> getCategories(Constants.BOARD_TYPE boardType, Locale locale) {
		return Category.list(boardType).stream().map(category -> new BoardCategory(
				category.name(),
				category.names.stream().filter(localSimpleName -> locale.getLanguage().equals(localSimpleName.getLanguage()))
     ...
     */
    public static BoardCategory getCategory(Constants.BOARD_TYPE boardType, String targetCategory, Locale locale) {
		if (Constants.BOARD_TYPE.FREE.equals(boardType)) {
			return null;
		} else {
			return Category.list(boardType).stream().filter(category -> category.equals(Category.valueOf(targetCategory))).findFirst().map(category -> new BoardCategory(
					category.name(),
					category.names.stream().filter(localSimpleName -> locale.getLanguage().equals(localSimpleName.getLanguage()))
							.collect(Collectors.toList()))
     ...
     */
    public static Boolean existCategory(Constants.BOARD_TYPE boardType, String targetCategory) {
    	if (Constants.BOARD_TYPE.FREE.equals(boardType)) {
    		return Boolean.TRUE;
		} else {
    		return Category.list(boardType).stream().anyMatch(category -> category.equals(Category.valueOf(targetCategory)));
		}
    }
